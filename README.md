...
- **编辑模式**：标题栏右上角铅笔按钮（或 `Ctrl+E`）进入源码编辑。左侧 Markdown 源码、右侧实时预览（同一套渲染管线）。可编辑正文/标题/列表/表格/链接、代码块、Mermaid 流程图、LaTeX 公式；可插入图片文件（写入文档旁 `images/`）或内嵌 data URI，并按语法边界删除光标处图片。`Ctrl+S` 保存，冲突时提示。
