...
- **编辑模式（Obsidian 风格 Live Preview）**：标题栏铅笔按钮（或 `Ctrl+E`）进入。文档以**最终渲染效果**呈现，点击任一段落/标题/代码/公式块即可就地编辑其 Markdown 源码，离开后恢复为最终效果（无左右分栏）。可编辑正文/标题/列表/表格/链接、代码块、Mermaid 流程图、LaTeX 公式；可插入图片文件（写入文档旁 `images/`）或内嵌 data URI，并按语法边界删除图片。`Ctrl+S` 保存，冲突时提示。真相源始终是完整 Markdown 文本，不从 HTML 反推。
...
| `Ctrl+E` | 进入 / 退出编辑模式 |
| `Ctrl+S` | 编辑模式中保存 |
| `Ctrl+Shift+P` | 编辑模式中刷新渲染 |
| `Ctrl+滚轮` | 正文中调整字号；图片 / Mermaid 灯箱中缩放内容 |
| `Ctrl++` / `Ctrl+-` / `Ctrl+0` | 灯箱中增量缩放 / 恢复适配；正文中调整 / 重置字号 |
